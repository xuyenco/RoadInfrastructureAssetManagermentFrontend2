...
namespace RoadInfrastructureAssetManagementFrontend2.Pages.Assets
{
    ...
    public class AssetImportModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetDownloadExcelUpdateTemplateAsync(int categoryId)
        {
            var username = HttpContext.Session.GetString("Username") ?? "anonymous";
            var role = HttpContext.Session.GetString("Role") ?? "unknown";

            _logger.LogInformation("User {Username} (Role: {Role}) is downloading Excel update template for category ID {CategoryId}",
                username, role, categoryId);

            if (categoryId == 0)
            {
                _logger.LogWarning("User {Username} (Role: {Role}) did not select a category for update template download",
                    username, role);
                return BadRequest("Vui lòng chọn danh mục");
            }
            ...
            using (var package = new ExcelPackage())
            {
                var worksheet = package.Workbook.Worksheets.Add($"Asset update template for {category.category_name}");
                var fixedColumns = new List<string>
                {
                    "asset_id", "category_id", "asset_name", "asset_code", "address", "geometry_coordinates", "geometry_system",
                    "construction_year", "operation_year", "land_area", "floor_area", "original_value", "remaining_value",
                    "asset_status", "installation_unit", "management_unit", "image_path"
                };
                ...
                worksheet.Cells[2, 2].Value = categoryId;

                worksheet.Cells[1, 1, 1, fixedColumns.Count + attributeColumns.Count].Style.Font.Bold = true;
                worksheet.Cells.AutoFitColumns();

                var stream = new MemoryStream(package.GetAsByteArray());
                _logger.LogInformation("User {Username} (Role: {Role}) successfully generated Excel update template for category ID {CategoryId}",
                    username, role, categoryId);
                return File(stream, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    $"Asset_Update_Template_Category_{categoryId}.xlsx");
            }
        }
        public async Task<IActionResult> OnPostImportUpdateExcelAsync(IFormFile excelFile, IFormFileCollection imageFiles)
        {
            ...
            _logger.LogInformation("User {Username} (Role: {Role}) is updating assets from Excel file", username, role);
            ...
            try
            {
                var assets = new List<(int asset_id, AssetsRequest asset)>();
                var errorRows = new List<ExcelErrorRow>();
                var imageFileMap = imageFiles.ToDictionary(f => f.FileName, f => f);
                    ...
                    using (var package = new ExcelPackage(stream))
                    {
                        ...
                        for (int row = 2; row <= rowCount; row++)
                        {
                            ...
                            for (int col = 1; col <= colCount; col++)
                            {
                                ...
                                {
                                    case "asset_id":
                                        assetId = int.TryParse(value, out var parsedId) ? parsedId : null; break;
                                    case "category_id":
                                        asset.category_id = int.TryParse(value, out var catId) ? catId : 0; break;
                                    case "asset_name":
                                        asset.asset_name = value; break;
                            ...
                            if (!assetId.HasValue)
                            {
                                _logger.LogWarning("User {Username} (Role: {Role}) missing or invalid asset_id in row {Row}",
                                    username, role, row);
                                errorRows.Add(new ExcelErrorRow
                                {
                                    RowNumber = row,
                                    OriginalData = JsonSerializer.Serialize(rowData),
                                    ErrorMessage = "Thiếu hoặc asset_id không hợp lệ."
                                });
                                continue;
                            }
                            ...
                            assets.Add((assetId.Value, asset));
                        }

                        int successCount = 0;
                        for (int i = 0; i < assets.Count; i++)
                        {
                            var (assetId, asset) = assets[i];
                            var rowNumber = i + 2;
                            ...
                            try
                            {
                                _logger.LogDebug("User {Username} (Role: {Role}) updating asset ID {Id} for row {Row}: {AssetData}",
                                    username, role, assetId, rowNumber, JsonSerializer.Serialize(asset));
                                var updatedAsset = await _assetsService.UpdateAssetAsync(assetId, asset);
                                if (updatedAsset == null)
                                {
                                    _logger.LogWarning("User {Username} (Role: {Role}) failed to update asset ID {Id} for row {Row}: No result returned",
                                        username, role, assetId, rowNumber);
                                    errorRows.Add(new ExcelErrorRow
                                    {
                                        RowNumber = rowNumber,
                                        OriginalData = JsonSerializer.Serialize(asset),
                                        ErrorMessage = $"Không thể cập nhật tài sản ID {assetId} (lỗi từ service)."
                                    });
                                    continue;
                                }

                                successCount++;
                                _logger.LogInformation("User {Username} (Role: {Role}) successfully updated asset ID {Id} for row {Row}",
                                    username, role, updatedAsset.asset_id, rowNumber);
                            }
                            catch (Exception ex)
                            {
                                _logger.LogWarning("User {Username} (Role: {Role}) encountered error updating asset ID {Id} for row {Row}: {Error}",
                                    username, role, assetId, rowNumber, ex.Message);
                                errorRows.Add(new ExcelErrorRow
                                {
                                    RowNumber = rowNumber,
                                    OriginalData = JsonSerializer.Serialize(asset),
                                    ErrorMessage = $"Lỗi khi cập nhật tài sản ID {assetId}: {ex.Message}"
                                });
                            }
                        }
                        ...
                        if (errorRows.Any())
                        {
                            ...
                            using (var errorPackage = new ExcelPackage())
                            {
                                ...
                                TempData["SuccessCount"] = successCount;
                                TempData["ErrorFile"] = Convert.ToBase64String(errorStream.ToArray());
                                _logger.LogInformation("User {Username} (Role: {Role}) updated {SuccessCount} assets with {ErrorCount} errors",
                                    username, role, successCount, errorRows.Count);
                            }
                        }
                        else
                        {
                            TempData["SuccessCount"] = successCount;
                            _logger.LogInformation("User {Username} (Role: {Role}) successfully updated {SuccessCount} assets with no errors",
                                username, role, successCount);
                        }
